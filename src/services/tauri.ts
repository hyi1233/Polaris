...
/** 获取工作区所有任务 */
export async function workspaceSchedulerGetTasks(
  workspacePath: string
): Promise<ScheduledTask[]> {
  return invoke<ScheduledTask[]>('workspace_scheduler_get_tasks', { workspacePath });
}

/** 获取工作区单个任务 */
export async function workspaceSchedulerGetTask(
  workspacePath: string,
  id: string
): Promise<ScheduledTask | null> {
  return invoke<ScheduledTask | null>('workspace_scheduler_get_task', { workspacePath, id });
}

/** 创建工作区任务 */
export async function workspaceSchedulerCreateTask(
  workspacePath: string,
  params: CreateTaskParams
): Promise<ScheduledTask> {
  return invoke<ScheduledTask>('workspace_scheduler_create_task', { workspacePath, params });
}

/** 更新工作区任务 */
export async function workspaceSchedulerUpdateTask(
  workspacePath: string,
  id: string,
  updates: Partial<ScheduledTask>
): Promise<ScheduledTask> {
  return invoke<ScheduledTask>('workspace_scheduler_update_task', { workspacePath, id, updates });
}

/** 删除工作区任务 */
export async function workspaceSchedulerDeleteTask(
  workspacePath: string,
  id: string
): Promise<ScheduledTask> {
  return invoke<ScheduledTask>('workspace_scheduler_delete_task', { workspacePath, id });
}

/** 获取工作区分页日志 */
export async function workspaceSchedulerGetLogs(
  workspacePath: string,
  page?: number,
  pageSize?: number
): Promise<PaginatedLogs> {
  return invoke<PaginatedLogs>('workspace_scheduler_get_logs', { workspacePath, page, pageSize });
}

/** 获取工作区任务日志 */
export async function workspaceSchedulerGetTaskLogs(
  workspacePath: string,
  taskId: string
): Promise<TaskLog[]> {
  return invoke<TaskLog[]>('workspace_scheduler_get_task_logs', { workspacePath, taskId });
}

/** 删除工作区任务日志 */
export async function workspaceSchedulerDeleteTaskLogs(
  workspacePath: string,
  taskId: string
): Promise<number> {
  return invoke<number>('workspace_scheduler_delete_task_logs', { workspacePath, taskId });
}

/** 获取工作区日志保留配置 */
export async function workspaceSchedulerGetRetentionConfig(
  workspacePath: string
): Promise<LogRetentionConfig> {
  return invoke<LogRetentionConfig>('workspace_scheduler_get_retention_config', { workspacePath });
}

/** 更新工作区日志保留配置 */
export async function workspaceSchedulerUpdateRetentionConfig(
  workspacePath: string,
  config: LogRetentionConfig
): Promise<LogRetentionConfig> {
  return invoke<LogRetentionConfig>('workspace_scheduler_update_retention_config', { workspacePath, config });
}
