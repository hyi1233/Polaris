...
// ============================================================================
// 工作区调度器命令（使用 SchedulerRepository）
// ============================================================================
...
/// 获取工作区所有任务
#[tauri::command]
pub fn workspace_scheduler_get_tasks(
    workspace_path: String,
) -> Result<Vec<ScheduledTask>> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.list_tasks()
}

/// 获取工作区单个任务
#[tauri::command]
pub fn workspace_scheduler_get_task(
    workspace_path: String,
    id: String,
) -> Result<Option<ScheduledTask>> {
    let repo = SchedulerRepository::new(&workspace_path);
...
/// 创建工作区任务
#[tauri::command]
pub fn workspace_scheduler_create_task(
    workspace_path: String,
    params: CreateTaskParams,
) -> Result<ScheduledTask> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.create_task(params)
}

/// 更新工作区任务
#[tauri::command]
pub fn workspace_scheduler_update_task(
    workspace_path: String,
    id: String,
    updates: crate::services::scheduler_repository::TaskUpdateParams,
) -> Result<ScheduledTask> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.update_task(&id, updates)
}

/// 删除工作区任务
#[tauri::command]
pub fn workspace_scheduler_delete_task(
    workspace_path: String,
    id: String,
) -> Result<ScheduledTask> {
    let repo = SchedulerRepository::new(&workspace_path);
...
/// 获取工作区分页日志
#[tauri::command]
pub fn workspace_scheduler_get_logs(
    workspace_path: String,
    page: Option<u32>,
    page_size: Option<u32>,
) -> Result<PaginatedLogs> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.list_logs(page.unwrap_or(1), page_size.unwrap_or(20))
}

/// 获取工作区任务日志
#[tauri::command]
pub fn workspace_scheduler_get_task_logs(
    workspace_path: String,
    task_id: String,
) -> Result<Vec<TaskLog>> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.get_task_logs(&task_id)
}

/// 删除工作区任务日志
#[tauri::command]
pub fn workspace_scheduler_delete_task_logs(
    workspace_path: String,
    task_id: String,
) -> Result<usize> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.delete_task_logs(&task_id)
}

/// 获取工作区日志保留配置
#[tauri::command]
pub fn workspace_scheduler_get_retention_config(
    workspace_path: String,
) -> Result<LogRetentionConfig> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.get_retention_config()
}

/// 更新工作区日志保留配置
#[tauri::command]
pub fn workspace_scheduler_update_retention_config(
    workspace_path: String,
    config: LogRetentionConfig,
) -> Result<LogRetentionConfig> {
    let repo = SchedulerRepository::new(&workspace_path);
    repo.update_retention_config(config)
}
